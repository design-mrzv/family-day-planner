...
// Чекбокс "виконано" — не ховає пункт (на відміну від "→ завтра"/moved), лише позначає.
// Дзеркало app/api/plan/move/route.ts — той самий патерн пошуку й збереження.
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return Response.json({ error: "bad_request", message: "Потрібні date, title і done." }, { status: 400 });
  }
  ...
  if (target) target.status = parsed.data.done ? "done" : undefined;
