...
# Етап 3 — доставка (Telegram, ІСТОРИЧНЕ — код видалено в Етапі 4)

⚠️ Розділи 10-11 описують Telegram-реалізацію Стадії 3. **Код видалено повністю**
(2026-09-15, після живого підтвердження, що push стабільно працює — реальна ранкова
доставка о 07:01 за Чикаго, точно в ціль, без ручного втручання): `app/api/telegram/**`,
`lib/telegram.ts`, колонка `users.telegram_chat_id`, таблиця `telegram_link_codes`
(окрема DROP-міграція `0005_slippery_avengers.sql`), env `TELEGRAM_BOT_TOKEN`/
`TELEGRAM_WEBHOOK_SECRET`, вебхук знято через `deleteWebhook` перед видаленням коду.
Лишено нижче для історії — чому й що саме замінено.

Ідентифікатор розширювався: email (Етап 2) лишався, Telegram **додавався** через
deep-link прив'язку — не замінював email-акаунт, а привʼязував `telegram_chat_id` до нього.
...
## 17. Статус Telegram-коду (закрито)

Telegram-код повністю видалено 2026-09-15 (розділи 10-11 — історія). Гейт підтверджено
живими даними з БД: `daily_plans.delivered_at` = реальна ранкова доставка о 07:01 за
Чикаго, точно в ціль, без ручного тригера. Push — єдиний канал доставки.

---

# Етап 4 — Read-only шеринг для партнера

`PRODUCT_SPEC_v2.md` розділ 4 крок 6, `BUILD_PLAN.md` Етап 4. Жорстка межа: партнер
**тільки дивиться**. Нуль акаунту, нуль редагування, нуль інтерактивних кнопок.

## 18. Схема, роути, публічна сторінка

```sql
users(
  ...,
  share_token_hash TEXT UNIQUE  -- NULL, поки не згенеровано; SHA-256 хеш, не сирий токен
)
```
Хеш, не сирий рядок — той самий принцип, що `magic_links.token_hash`: токен
довгоживучий і дає доступ до розкладу, тому за витоку БД лишається непридатним.
На відміну від magic link — без `used_at`, лишається дійсним, поки не перегенерований
(`lib/share.ts` — `hashShareToken`, спільна для генерації й резолюції).

**`POST /api/share/token`** (вимагає сесію) → `{ "shareUrl": "https://.../share/<токен>" }`.
Генерує `randomBytes(24)`, перезаписує `share_token_hash` (це і є "перегенерувати" —
стара версія посилання одразу відмирає). Сирий токен показується **один раз**,
повторно недоступний (як API-ключ).

**`app/share/[token]/page.tsx`** — перший dynamic route в проєкті, публічна Server
Component-сторінка, без сесії. Хешує токен з URL → шукає `users` за
`share_token_hash` → нема → `notFound()` (стандартний Next 404). Є → рахує
`dateStringInTz(user.timezone, 0)` (пояс **власниці**, не гостя — у партнера немає
ні сесії, ні свого поясу) → шукає `daily_plans` на цю дату → рендерить
...
## 20. Ручний колір задачі, "Перенести → сьогодні", чіпи prefill (Етап 5, раунд 4)

**Колір задачі** — чисто візуальний, НЕ категоризація (LLM як і раніше нічого
не класифікує). За замовчуванням клієнт рахує детермінований хеш
`normalizeTaskKey(title)` → 1 з 6 (`lib/color.ts`: `resolveColorIndex`, чиста
функція, без `lib/db` — цей файл імпортують клієнтські компоненти). Людина може
свідомо закріпити свій колір у `TaskDetailSheet` — тоді override з БД виграє.

`taskColors` (`task_colors`) — точна копія `durationOverrides`: `user_id`,
`task_key`, `color_index` (1-6), `unique(user_id, task_key)`.

**`GET /api/task-color`** → `{ "colors": { "<taskKey>": 3, ... } }` — усі
override користувача одним запитом, підвантажується в `Planner.tsx` при
монтуванні. **`POST /api/task-color`** — `{ "title": "...", "color_index": 1..6 }`,
upsert (inline у роуті — на відміну від `duration-override`, тут лише один
викликач, спільний `lib/*.ts`-хелпер не виправданий).

Обсяг: тільки авторизований перегляд. Публічна `share/[token]` сторінка не має
сесії → бачить лише авто-хеш, override туди не підвантажується.

**`POST /api/plan/move-to-free-slot`** — вхід `{ date, title }` (як `move`).
Знаходить задачу, шукає `findFreeSlot(duration_min, occupied)` серед решти
розкладу дня (той самий виклик, що вже використовує `reschedule` для
конфліктів — не нова логіка). Знайдено → оновлює `start`, повертає повний
...
**Не раніше за "зараз"** — обидві гілки передають `minutesInTz(timezone)`
(`lib/timezone.ts`) як нижню межу для гнучких задач: `solve()` і
`placeNewTasks` отримали опційний параметр `minStartMinutes` (за
замовчуванням `0` — вечірнє планування на завтра не зачеплене), який
підіймає нижню межу вікна розміщення гнучкої справи (і зсуву конфліктної
існуючої задачі через `findFreeSlot`) до поточного моменту дня. Без цього
FAB "Сьогодні" міг поставити щойно додану гнучку задачу вранці (07:00),
навіть якщо зараз, наприклад, 11:12 — реальний живий баг, знайдений
користувачем.

## 22. Попап "не вистачило вільного часу" (Етап 5, раунд 7)

Реальний живий баг: коли конфліктну задачу зсуває через `findFreeSlot`
(розділ 19/21) і вільного місця не знаходиться, вона падає в `overflow` з
`reason: "no_slot"` — **мовчки** з погляду людини. Банер підтвердження
("Перенести її на вільний час?") виглядає як обіцянка успіху, хоча solver
чесно міг не знайти куди її поставити.

`placeNewTasks` (`lib/solver/addTasks.ts`) тепер повертає при успіху
`{ ok: true, displaced: string[] }` замість просто `{ ok: true }` —
`displaced` це назви задач, які довелось прибрати зі `schedule` в
`overflow` за час цього виклику. `POST /api/plan/reschedule` рахує той
самий список локально (`displaced: string[]`) і, як і `add-today`, додає
його поруч із рештою `SolverResult` у відповідь: `{ ...SolverResult,
"displaced": string[] }` — не окрема обгортка, щоб фронтенд і далі міг
кастити відповідь як `SolverResult` без змін для решти полів.

`app/Planner.tsx`: новий стан `displacedNotice: string[] | null`.
`onAddToday` перевіряє `data.displaced` після успішного збереження;
`TaskDetailSheet`'s `onSaved` отримав другий опційний аргумент
`displaced?: string[]`. Непорожній список → попап (той самий
`.modal-backdrop`/`.modal-panel`, що інші модалки — не `alert()`, щоб не
випадати з дизайн-мови застосунку): "«X» довелось перенести в «Не влізло
сьогодні» — вільного часу поруч не знайшлося." + кнопка "Зрозуміло".

## 23. Видалення рутинних задач із пропозицій (Етап 5, раунд 8)

Поріг рутини (≥2 з останніх 7 днів, розділ 9) уже сам "забуває" старе —
ковзне вікно. Те, що виглядало як переобсяг пропозицій, насправді був
**carry** (усе незакрите з учорашнього дня, без порогу). Людина хоче
постійно прибирати конкретну назву з пропозицій незалежно від джерела
(carry чи routine).

`ignored_routines` (`lib/db/schema.ts`) — точна копія `task_colors` без
"значення": `user_id`, `task_key`, `unique(user_id, task_key)`. Сама
наявність рядка — прапорець "не пропонувати".

`computeRoutine`/`computePrefill` (`lib/routine.ts`) отримали третій
опційний параметр `ignored: Set<string> = new Set()` (normalizeTaskKey-
ключі) — фільтрує і carry (`uniqueTitles`), і кандидатів рутини до
підрахунку `count`. Не нова логіка рутини, лише фільтр на вже існуючих
шляхах.

**`GET /api/routine`** підвантажує `ignored_routines` користувача одним
запитом поруч із `daily_plans`, будує `Set`, передає в `computePrefill`.

**`POST /api/routine/ignore`** (новий) — вхід `{ "title": string }` →
`normalizeTaskKey` → `insert ... onConflictDoNothing` (ідемпотентно).
Відповідь `{ ok: true }`. Без GET — клієнту потрібен лише вже
відфільтрований `/api/routine`, не сирий ігнор-список.

Фронтенд: кожен пункт дропдауна "Рутинні задачі" (`app/TaskInputSheet.tsx`)
отримав кнопку-кошик поруч із чекбоксом (не всередині `<label>`, щоб клік
не перемикав вибір) → модалка підтвердження (той самий шаблон, що попап
"Не вистачило вільного часу", розділ 22) → підтвердження викликає
`Planner.tsx`'s `onIgnoreRoutineItem`, який зберігає на бекенді й
оптимістично прибирає назву з `routineItems`/`selectedRoutine`.

Ігнор **постійний**, не "на сьогодні" — свідомий компроміс. Впливає лише
на обчислення підказки: ручний ввід тієї самої назви в textarea і далі
успішно планується як завжди.

## 24. "Завтра" — інкрементне домержування замість повного перезапису (Етап 5, раунд 9)

Реальний живий баг: людина вписує задачі по одній протягом вечора — кожен
виклик "Розкласти план" (`POST /api/plan`) робив повний перезапис дня
(`planAndSaveDay`), і попередньо введена задача губилась, якщо не потрапила
в чіпи рутини. `POST /api/plan` тепер має той самий дуал-режим, що вже має
`POST /api/plan/add-today` (розділ 21): перевіряє, чи є рядок `daily_plans`
для цільової дати (`today` з тіла запиту — те саме поле, що й раніше, і далі
приходить від клієнта, не рахується на сервері — це вже була поведінка
роута, не змінено).

- **Нема рядка** → як завжди, `planAndSaveDay(userId, text, today)`, повний
  план з нуля.
- **Є рядок** → `parseTasks(text, today)` (LLM парсить лише новий текст) →
  `placeNewTasks` (`lib/solver/addTasks.ts`, той самий виклик, що
  `add-today`) домержовує в existing, БЕЗ `minStartMinutes` (default `0`) —
  "завтра" не має обмеження "не раніше за зараз", на відміну від "сьогодні".
  Конфлікт і без `resolve_conflict` → `409 { error: "time_conflict",
  conflicts }`. Успіх → `{ ...SolverResult, displaced: string[] }`
  (той самий формат, що `add-today`/`reschedule` — попап "Не вистачило
  вільного часу", розділ 22, працює й тут без жодних змін).

**`placeNewTasks` тепер не дублює вже наявні назви**: на початку збирає
`Set` нормалізованих ключів з `existing.schedule` (без "moved") +
`existing.overflow`, і пропускає (мовчазний no-op, без помилки й без запису
в overflow) будь-якого нового кандидата, чий ключ вже там є. Потрібно саме
для "Завтра" — чіпи рутини лишаються позначеними між відкриттями модалки
(`selectedRoutine` у `Planner.tsx` не скидається після сабміту), тож без
цього фільтра повторний виклик з тими самими чіпами додав би їх ще раз.
Дедлайни не чіпаємо (чіпи рутини їх не включають).

`app/Planner.tsx`'s `onPlan` тепер повертає той самий формат, що
`onAddToday`: `{ok, message?, conflicts?}` замість `Promise<boolean>`.
`app/TaskInputSheet.tsx`: банер конфлікту (раніше лише для вкладки
"Сьогодні") тепер спільний для обох вкладок — той самий локальний стан
`conflict`, кнопка підтвердження викликає `handleAddToday(true)` або
`handlePlan(true)` залежно від активної вкладки.

## 25. Велика літера в назвах задач (Етап 5, раунд 9)

Чисто візуальне: `.cap-first::first-letter { text-transform: uppercase; }`
(`app/globals.css`) — НЕ `text-transform: capitalize` (той міняє регістр
кожного слова, а не лише першої літери речення). Застосовано на назвах у
`app/ScheduleView.tsx`: timeline-картка, рядок overflow, рядок дедлайну.
Збережений текст, `normalizeTaskKey`, пошук і дедуп — усе працює з
оригінальним регістром, як і раніше; змінюється лише те, що бачить око.
Поле "Назва" в `TaskDetailSheet.tsx` (редагування) навмисно без змін —
там людина бачить і редагує сирий текст.
