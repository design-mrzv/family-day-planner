# CONTRACT.md — контракт даних (Етапи 1-4)

> Джерело істини для меж між компонентами. Заморожено й узгоджено до коду.
> Змінювати тільки свідомо й з окремим комітом. Продуктовий контекст: `PRODUCT_SPEC_v2.md`,
> порядок білду: `BUILD_PLAN.md`.

## Архітектурне правило (не переговорюється)

- **LLM тільки парсить текст → JSON.** LLM НЕ оцінює тривалості й НЕ розкладає час.
  Виняток (Етап 5, раунд 13, розділ 28): якщо людина сама явно написала число хвилин
  у тексті — LLM його ВИТЯГУЄ (`duration_min`), той самий клас дії, що `fixed_time`.
  Це не оцінка/вгадування, тому не порушує правило.
- **Solver — детермінований код** (цикли й перевірки). Тривалості призначає solver зі
  словника дефолтів, якщо в тексті нічого явно не вказано. Без зовнішніх
  solver-бібліотек, без оптимізації.
- Вивід LLM валідуємо схемою. Смітт → один ретрай → чесна помилка. Без другої моделі
  для ПЕРЕВІРКИ виводу (LLM-суддя заборонений). Резервна модель на випадок
  перевантаження основної (розділ 26) — не порушення цього правила: вона виконує ту
  саму роботу парсингу, коли основна недоступна, не звіряє чужий результат.

## Формати (скрізь)

- Час: `"HH:MM"`, 24-годинний (напр. `"08:00"`, `"15:00"`).
- Дата: `"YYYY-MM-DD"` (напр. `"2026-09-08"`).

---

## 1. Вхід у LLM-парсер

```json
{ "text": "сирий текст мами", "today": "2026-09-05" }
```

## 2. Вихід LLM-парсера (строга схема, нічого крім цього JSON)

```json
{
  "tasks": [
    { "title": "забрати старшого", "fixed_time": "15:00", "deadline": null, "time_hint": null, "duration_min": null },
    { "title": "тренування",       "fixed_time": null,    "deadline": null, "time_hint": null, "duration_min": null },
    { "title": "оплатити садок",    "fixed_time": null,    "deadline": "2026-09-08", "time_hint": null, "duration_min": null },
    { "title": "щось приготувати", "fixed_time": null,    "deadline": null, "time_hint": "evening", "duration_min": null },
    { "title": "заняття з логопедом", "fixed_time": null, "deadline": null, "time_hint": null, "duration_min": 30 }
  ]
}
```

Правила поля:
- `title` — рядок, обовʼязковий, непорожній. Без слів-маркерів часу доби чи тривалості
  всередині (ідуть у `time_hint`/`duration_min`).
- `fixed_time` — `"HH:MM"` або `null`.
- `deadline` — `"YYYY-MM-DD"` або `null`. **Тільки для реальної календарної дати/дня тижня.**
  Розмита вказівка часу протягом сьогодні ("до 15", "до вечора") — НЕ deadline, `null`.
- `time_hint` — `"morning" | "afternoon" | "evening" | null`. LLM фіксує **лише буквально
  сказане** в тексті ("зранку"/"ввечері"/"на ніч" тощо), ніколи не вгадує з суті справи.
  Немає слова в тексті → `null`, навіть якщо справа "звучить" як вечірня.
- `duration_min` — число хвилин (5-480) або `null` (Етап 5, раунд 13). **ВИТЯГУВАННЯ, не
  ОЦІНКА** — LLM фіксує лише явно написане людиною число ("(30 хв)", "на 30 хвилин",
  "1 година"), ніколи не вгадує тривалість із суті справи (той самий принцип, що
  `time_hint`). Це не порушує заборону "LLM не оцінює тривалість" нижче — та заборона
  про самостійне ВГАДУВАННЯ моделлю, а не про читання готового факту з тексту.
  Немає числа в тексті → `null`, і тоді тривалість призначає лише solver, як завжди.

## 3. Вхід у solver

`tasks[]` (розділ 2) + `rules` + `durations`.
На Етапі 1: `rules` — хардкод, `durations` — словник дефолтів + fallback 30 хв.
Це внутрішні константи solver-а (не міжкомпонентний контракт); форму фіксуємо в 1.4.

## 4. Вихід solver-а = успішна відповідь `/api/plan`

```json
{
  "schedule": [
    { "title": "тренування",      "start": "08:00", "duration_min": 60, "type": "flexible" },
    { "title": "забрати старшого", "start": "15:00", "duration_min": 30, "type": "fixed" }
  ],
  "overflow":  [ { "title": "зняти відео", "duration_min": 40, "reason": "no_slot" } ],
  "deadlines": [ { "title": "оплатити садок", "date": "2026-09-08" } ]
}
```

Правила поля:
- `schedule[].type` — `"fixed" | "flexible"`.
- `overflow[].reason` — `"no_slot" | "conflict"`. Один масив `overflow` на всі причини.
- `deadlines[]` — усе, де `deadline != null`.

## 5. Відповідь `/api/plan` при помилці

```json
{ "error": "parse_failed", "message": "Не вдалося розібрати текст. Спробуй ще раз." }
```

---

## Крайові правила (зафіксовані рішення)

1. `deadline != null` → справа йде в `deadlines[]`, **не** в `schedule`.
2. Справа має і `fixed_time`, і `deadline` одночасно → `fixed_time` виграє: ставимо в
   розклад на сьогодні, `deadline` ігноруємо (рідкісний edge, на Етапі 1 не ускладнюємо).
3. Конфлікт двох `fixed_time` (той самий слот) → **обидві** справи в `overflow` з
   `reason: "conflict"`. Жодна не займає слот довільно.
4. Overflow — першокласний очікуваний стан, не помилка. Нічого не стискаємо й не викидаємо
   мовчки.
5. `time_hint` — фолбек. Якщо `title` збігається з якимось хардкод-правилом solver-а
   (напр. "тренування" → зранку) — правило за ключовим словом виграє над `time_hint`.

---

# Етап 2 — памʼять

Ідентифікатор: email (magic link, без пароля). Зберігання: Vercel Postgres (Neon) +
Drizzle ORM. Авторизація: самопис (`jose`, HS256), не NextAuth — свідомий вибір під дух
проєкту («проста функція», без важких фреймворків там, де вистачає власного коду).

## 6. Схема БД (`lib/db/schema.ts`)

```sql
users(
  id UUID PK,
  email TEXT UNIQUE NOT NULL,
  created_at TIMESTAMPTZ
)

magic_links(
  id UUID PK,
  email TEXT NOT NULL,
  token_hash TEXT NOT NULL,      -- SHA-256 сирого токена; сирий токен ніде не зберігається
  expires_at TIMESTAMPTZ NOT NULL,  -- TTL 15 хв
  used_at TIMESTAMPTZ,           -- NULL поки не використаний; заповнюється при verify (replay-захист)
  created_at TIMESTAMPTZ
)

daily_plans(
  id UUID PK,
  user_id UUID FK → users,
  date DATE NOT NULL,
  input_text TEXT NOT NULL,      -- сирий ввід мами (для дебагу й аудиту)
  tasks JSONB NOT NULL,          -- повний SolverResult { schedule, overflow, deadlines };
                                  -- schedule[].status?: "moved" (крок 4 спеку)
  created_at TIMESTAMPTZ,
  updated_at TIMESTAMPTZ,
  UNIQUE(user_id, date)          -- один запис на день; UPSERT перезаписує
)

duration_overrides(
  id UUID PK,
  user_id UUID FK → users,
  task_key TEXT NOT NULL,        -- normalizeTaskKey(title): lowercase + trim, точний збіг
  duration_min INT NOT NULL,
  updated_at TIMESTAMPTZ,
  UNIQUE(user_id, task_key)
)
```

Міграції: `drizzle-kit generate` (SQL у `drizzle/`, комітиться) + `drizzle-kit migrate`
(застосовує до `POSTGRES_URL_NON_POOLING`). Голий CLI не читає `.env.local` — запускати
через `node --env-file=.env.local node_modules/.bin/drizzle-kit ...`.

## 7. Авторизація (magic link)

**`POST /api/auth/request-link`** — вхід: `{ "email": "mama@example.com" }`.
Генерує токен (32 байти, base64url), зберігає лише його SHA-256 хеш. У dev-режимі (email
ще не шлеться) повертає посилання прямо у відповіді:
```json
{ "ok": true, "devLink": "https://.../api/auth/verify?token=..." }
```

**`GET /api/auth/verify?token=...`** — перевіряє токен (не протермінований, не використаний
раніше), позначає використаним, створює користувача за email якщо новий, підписує сесію
(JWT HS256, `AUTH_SECRET`, 30 днів) і редиректить на `/` з httpOnly cookie `session`.
Невалідний/повторний токен → редирект на `/?auth_error=1`.

**`POST /api/auth/logout`** — очищує cookie `session`.

**Усі інші `/api/*` (крім auth-роутів)** тепер вимагають сесію:
```json
{ "error": "unauthorized", "message": "Потрібно увійти." }
```
з кодом `401`, якщо cookie відсутній/невалідний.

## 8. Памʼять тривалостей

**`POST /api/duration-override`** — вхід: `{ "title": "вечеря", "duration_min": 60 }`
(додатне ціле, ≤480). UPSERT у `duration_overrides` за `(user_id, normalizeTaskKey(title))`.

Solver (`solve(tasks, durationOverrides?)`) приймає другим аргументом
`Map<normalizeTaskKey(title), duration_min>`; якщо є збіг — виграє над дефолтним словником
(`lib/solver/config.ts`). Порожня Map за замовчуванням = поведінка Етапу 1.

`POST /api/plan` тепер, крім парсингу й розкладки: (1) підвантажує всі
`duration_overrides` користувача перед `solve()`; (2) після — UPSERT `daily_plans`
за `(user_id, today)`.

## 9. Памʼять рутини + «→ завтра»

**`GET /api/routine`** → `{ "prefill": "текст для textarea" }`. Логіка (`lib/routine.ts`,
чиста функція, без LLM):
1. Бере останні 7 записів `daily_plans` користувача.
2. **carry** — усі унікальні назви справ з **останнього** дня (schedule+overflow, дедуп
   за `normalizeTaskKey`) — включно з тими, що мають `status: "moved"`.
3. **routine** — назви, що зустрічались у **≥2 різних днях** з цих 7 (дедлайни не рахуються
   — вони одноразові).
4. Результат = carry + routine, дедуп за назвою (carry має пріоритет, routine нічого не
   дублює). **Час (`fixed_time`) не переноситься** — тільки текст назви.

**`POST /api/plan/move`** — вхід: `{ "date": "YYYY-MM-DD", "title": "..." }`. Знаходить
справу з цим `title` у `daily_plans.tasks.schedule` за вказану дату, ставить
`status: "moved"`. UI ховає такі справи з поточного дня; наступного дня вони природно
потрапляють у `carry` (крок 4 спеку: «незакрите мовчки падає в завтрашній інбокс»,
без лічильників і осуду).

---

# Етап 3 — доставка (Telegram, ІСТОРИЧНЕ — код видалено в Етапі 4)

⚠️ Розділи 10-11 описують Telegram-реалізацію Стадії 3. **Код видалено повністю**
(2026-09-15, після живого підтвердження, що push стабільно працює — реальна ранкова
доставка о 07:01 за Чикаго, точно в ціль, без ручного втручання): `app/api/telegram/**`,
`lib/telegram.ts`, колонка `users.telegram_chat_id`, таблиця `telegram_link_codes`
(окрема DROP-міграція `0005_slippery_avengers.sql`), env `TELEGRAM_BOT_TOKEN`/
`TELEGRAM_WEBHOOK_SECRET`, вебхук знято через `deleteWebhook` перед видаленням коду.
Лишено нижче для історії — чому й що саме замінено.

Ідентифікатор розширювався: email (Етап 2) лишався, Telegram **додавався** через
deep-link прив'язку — не замінював email-акаунт, а привʼязував `telegram_chat_id` до нього.

## 10. Схема БД (доповнення до розділу 6)

```sql
users(
  ...,                              -- як у розділі 6
  telegram_chat_id TEXT UNIQUE,     -- NULL, поки не привʼязано
  timezone TEXT NOT NULL DEFAULT 'Europe/Kyiv'  -- IANA, явно задається /timezone
)

telegram_link_codes(
  id UUID PK,
  user_id UUID FK → users,
  code TEXT UNIQUE NOT NULL,        -- 12 hex-символів (randomBytes(6))
  expires_at TIMESTAMPTZ NOT NULL,  -- TTL 15 хв
  used_at TIMESTAMPTZ,              -- replay-захист, як magic_links
  created_at TIMESTAMPTZ
)

daily_plans(
  ...,                              -- як у розділі 6
  delivered_at TIMESTAMPTZ          -- NULL, поки не надіслано вранці;
                                     -- захист від дубля при ретраї cron
)
```

## 11. Прив'язка Telegram (deep-link)

**`POST /api/telegram/link`** (вимагає сесію) → `{ "deepLink": "https://t.me/<bot>?start=<код>" }`.
Генерує одноразовий код у `telegram_link_codes`, TTL 15 хв.

**`POST /api/telegram/webhook`** — приймає Telegram Update. Захист: заголовок
`X-Telegram-Bot-Api-Secret-Token` має збігатися з `TELEGRAM_WEBHOOK_SECRET`, інакше `401`.
Завжди повертає `{ "ok": true }` (Telegram не має ретраїти через наші внутрішні помилки —
вони логуються, не проброшуються назовні).

Розбір вхідного тексту:
- `/start <код>` → шукає код у `telegram_link_codes` (не використаний, не протермінований),
  ставить `used_at`, пише `telegram_chat_id` в `users`. Конфлікт (chat вже привʼязаний до
  іншого акаунта, код БД `23505`) → чесне повідомлення, **не** плутати з іншими помилками.
- `/timezone [місто|країна|IANA|+N]` — див. розділ 13.
- будь-який інший текст → вечірній ввід, трактується як справи на **завтра** (за
  `users.timezone` цього користувача): `planAndSaveDay(userId, text, dateStringInTz(tz, 1))`.
  Коротке підтвердження в чат («Записала N справ»), **не** повний розклад — той приходить
  окремо вранці (двотактний цикл, PRODUCT_SPEC_v2 розділ 4).

## 12. Cron — вечірній пінг і ранкова видача (ІСТОРИЧНЕ — замінено в Етапі 4)

⚠️ Цей розділ описує механізм Етапу 3 (фіксований раз-на-добу Vercel cron,
`telegram_chat_id`). **Він більше не діє** — `/api/cron/*` тепер працюють по-іншому,
див. розділ 14. Лишено для історії (чому взагалі виникла потреба в Етапі 4).

Старий механізм: `vercel.json` з двома завданнями раз на добу за фіксованим UTC-часом
(~19:00/~07:00 Europe/Kyiv), розсилка через `telegram_chat_id`. Проблема: для
користувача не за Києвом (напр. Чикаго) фіксований час — це або середина дня, або
глибока ніч за його реальним годинником, не вечір/ранок. Виявлено живим тестуванням
(розділ 14 — чому саме зовнішній погодинний тригер, а не просто інший фіксований час).

## 13. Часовий пояс (`resolveTimezone`, `lib/timezone.ts`)

Telegram не передає часовий пояс користувача — **ніколи не вгадуємо** (той самий
принцип, що й `time_hint` у розділі 2). Приймає (через `POST /api/timezone` у вебі —
Етап 4, раніше через команду `/timezone` в боті), у порядку спроб:

1. Повна IANA-назва як є (`America/Chicago`) — якщо `Intl` її вже визнає валідною.
2. Аліас-словник (`CITY_ALIASES`): столиці й великі міста кирилицею (латиниця здебільшого
   знаходиться прямим пошуком по IANA — але кирилиця з латиницею НІКОЛИ не збігаються
   посимвольно, навіть якщо місто там є); країни з **одним** поясом на всю територію
   (Україна, Німеччина, Польща...) — свідомо БЕЗ багатозонних (США/Канада/Австралія
   тощо), там нема єдиної правильної відповіді; великі міста США/Канади без власного
   запису в IANA (Х'юстон, Маямі — однозначні, на відміну від країни).
3. Пошук по всій базі `Intl.supportedValuesOf("timeZone")` за співпадінням назви міста.
4. Числовий зсув (`+2`, `UTC-5`, `GMT+3`) → `Etc/GMT∓N` (в IANA цей запис зі свідомо
   **інвертованим** знаком). Універсальний фолбек для будь-якої точки на Землі.

Нічого не впізнано → `null`, чесне повідомлення з підказками, не приблизний здогад.
Апострофи в транслітерації (`Х'юстон`/`Х’юстон`/`Х‘юстон`) нормалізуються перед
пошуком — інакше різні клавіатури дають різний символ і матчинг ламається.

---

# Етап 4 — Web Push (заміна Telegram)

Причина: Етап 3 виявив фундаментальний розрив — Vercel Cron фіксований на глобальному
UTC-часі, а користувач хоче реальну доставку о своїй **локальній** ранковій/вечірній
годині. **Vercel Hobby дозволяє нативний крон не частіше разу на добу** (офіційно
задокументовано, частіший вираз провалює деплой) — per-user timezone логіка на ньому
неможлива. Рішення: зовнішній безкоштовний погодинний тригер + канал доставки —
web push замість Telegram (рішення користувача: не тримати окремий бот-застосунок,
увесь ввід/вивід — у веб).

## 14. Схема БД (доповнення до розділу 6/10)

```sql
users(
  ...,
  last_evening_ping_date DATE  -- дедуп вечірнього пінгу, той самий принцип, що
                                 -- daily_plans.delivered_at для ранкової видачі
)

push_subscriptions(
  id UUID PK,
  user_id UUID FK → users,
  endpoint TEXT UNIQUE NOT NULL,  -- ідентифікатор підписки браузера; по суті замінює
                                    -- telegram_chat_id, але користувач може мати кілька
                                    -- (кілька пристроїв) — на відміну від одного чату
  p256dh TEXT NOT NULL,           -- ключі шифрування payload, вимагає Push API
  auth TEXT NOT NULL,
  created_at TIMESTAMPTZ
)
```

## 15. Підписка на push

**`GET /api/timezone`** / **`POST /api/timezone`** (вимагає сесію) — читання й запис
`users.timezone`, веб-заміна команди `/timezone` (розділ 13).

**`POST /api/push/subscribe`** (вимагає сесію) — вхід: `PushSubscription.toJSON()`
з браузера (`{ endpoint, keys: { p256dh, auth } }`). UPSERT за `endpoint` (глобально
унікальний, той самий пристрій повторно підписується поверх себе).

Клієнт (`app/Planner.tsx`): кнопка «Увімкнути сповіщення» → `serviceWorker.register("/sw.js")`
→ `Notification.requestPermission()` → `pushManager.subscribe()` (VAPID public key,
`NEXT_PUBLIC_VAPID_PUBLIC_KEY`) → POST на `/api/push/subscribe`.

**iOS Safari обмеження (системне, Apple, не обійти кодом):** push працює лише для
сайтів, доданих на головний екран (iOS 16.4+). Детект (`navigator.userAgent` +
`navigator.standalone`) показує інструкцію "Поділитися → На головний екран" замість
кнопки, коли сайт відкрито у звичайній вкладці Safari.

`app/manifest.ts` + `public/icon.png` — мінімальний маніфест для installability
(Android/Chrome "Install", iOS "На головний екран"). Іконка — суцільний квадрат,
суто функціональна, не дизайн-рішення (дизайн — Етап 5).

## 16. Погодинний тригер (заміна розділу 12)

`vercel.json` **видалено** (native cron більше не використовується). Перша спроба —
**GitHub Actions** (`schedule: "0 * * * *"`) — виявилась ненадійною: GitHub офіційно
попереджає, що `schedule`-запуски best-effort, і для тихих/нових репо спостерігались
затримки на кілька годин (підтверджено живим тестом: розрив між тіками сягав 3-5 год
замість обіцяної години). **Замінено на [cron-job.org](https://cron-job.org)**
(безкоштовний, спеціалізований HTTP-крон-сервіс, до 60 запусків/год) — два завдання
(`family-planner: evening-ping`, `family-planner: morning-delivery`), кожне погодинно
(`minutes: [0]`, `hours: [-1]` — щогодини), б'ють `GET` у наші ендпоінти з кастомним
заголовком `Authorization: Bearer $CRON_SECRET` (через `extendedData.headers`).
Налаштовано через їхній REST API (`PUT https://api.cron-job.org/jobs`), не вручну
через UI.

Роути (`app/api/cron/evening-ping`, `app/api/cron/morning-delivery`) самі фільтрують,
кому зараз слати — `hourInTz(user.timezone)` (`lib/timezone.ts`) порівнюється з
цільовою годиною (19 увечері, 7 вранці). Джерело користувачів — `push_subscriptions`
(join, не `telegram_chat_id`). Дедуп: `users.last_evening_ping_date` (evening) /
`daily_plans.delivered_at` (morning) — обидва виставляються лише якщо хоч одна push-
відправка користувачу реально вдалась.

`lib/push.ts` — `sendPush()`: 404/410 від браузера (підписка мертва — вимкнули
сповіщення/видалили PWA) мовчки прибирає рядок з `push_subscriptions`, це не помилка.

## 17. Статус Telegram-коду (закрито)

Telegram-код повністю видалено 2026-09-15 (розділи 10-11 — історія). Гейт підтверджено
живими даними з БД: `daily_plans.delivered_at` = реальна ранкова доставка о 07:01 за
Чикаго, точно в ціль, без ручного тригера. Push — єдиний канал доставки.

---

# Етап 4 — Read-only шеринг для партнера

`PRODUCT_SPEC_v2.md` розділ 4 крок 6, `BUILD_PLAN.md` Етап 4. Жорстка межа: партнер
**тільки дивиться**. Нуль акаунту, нуль редагування, нуль інтерактивних кнопок.

## 18. Схема, роути, публічна сторінка

```sql
users(
  ...,
  share_token_hash TEXT UNIQUE  -- NULL, поки не згенеровано; SHA-256 хеш, не сирий токен
)
```
Хеш, не сирий рядок — той самий принцип, що `magic_links.token_hash`: токен
довгоживучий і дає доступ до розкладу, тому за витоку БД лишається непридатним.
На відміну від magic link — без `used_at`, лишається дійсним, поки не перегенерований
(`lib/share.ts` — `hashShareToken`, спільна для генерації й резолюції).

**`POST /api/share/token`** (вимагає сесію) → `{ "shareUrl": "https://.../share/<токен>" }`.
Генерує `randomBytes(24)`, перезаписує `share_token_hash` (це і є "перегенерувати" —
стара версія посилання одразу відмирає). Сирий токен показується **один раз**,
повторно недоступний (як API-ключ).

**`app/share/[token]/page.tsx`** — перший dynamic route в проєкті, публічна Server
Component-сторінка, без сесії. Хешує токен з URL → шукає `users` за
`share_token_hash` → нема → `notFound()` (стандартний Next 404). Є → рахує
`dateStringInTz(user.timezone, 0)` (пояс **власниці**, не гостя — у партнера немає
ні сесії, ні свого поясу) → шукає `daily_plans` на цю дату → рендерить
`<ScheduleView result={...} readOnly />` або "Плану на сьогодні ще немає."

**`app/ScheduleView.tsx`** — презентаційний компонент, спільний для `Planner.tsx`
(readOnly=false — чекбокс "виконано" + тап по картці відкриває деталі, Етап 5,
розділ 19) і публічної сторінки (readOnly=true — жодних інтерактивних елементів).
Одна розмітка, два режими.

Посилання **постійне** (партнер бекмаркає раз) — показує поточний "сьогодні" при
кожному відкритті, не статичний знімок одного дня.

## 19. Ручне редагування часу задачі + конфлікт (Етап 5)

Timeline-картка (`app/ScheduleView.tsx`) показує лише назву + час + чекбокс
"виконано". Тап (не readOnly) відкриває `app/TaskDetailSheet.tsx` — назва,
початок/кінець, опційний чекбокс "запам'ятати тривалість для схожих задач" (той
самий механізм, що розділ 8 — той самий upsert, винесений у `lib/duration.ts`:
`saveDurationOverride`, перевикористовується і `api/duration-override`, і цим
роутом).

**`POST /api/plan/reschedule`** — вхід:
```json
{
  "date": "YYYY-MM-DD",
  "title": "поточна назва (ключ пошуку)",
  "new_title": "назва після редагування",
  "start": "HH:MM",
  "duration_min": 60,
  "remember_duration_min": 45,
  "resolve_conflict": true
}
```
`remember_duration_min` і `resolve_conflict` — опційні. Знаходить елемент у
`daily_plans.tasks.schedule` за `normalizeTaskKey(title)` (як `move`/`complete`).

Якщо нове `[start, start+duration_min]` перетинається з іншою задачею дня
(буквальний timeline-overlap, `lib/solver/solve.ts`: `overlaps`, без буфера) і
`resolve_conflict` не передано — **нічого не зберігає**, відповідає `409`:
```json
{ "error": "time_conflict", "conflicts": [{ "title": "...", "start": "HH:MM", "duration_min": 60 }] }
```
Фронтенд показує банер із підтвердженням, повторює запит з `resolve_conflict: true`.

З `resolve_conflict: true` — кожну конфліктну задачу зсуває на вільний слот через
`findFreeSlot(durationMin, occupied)` (`lib/solver/solve.ts`, тонка обгортка над
`placeFlexible` — **той самий** пошук, що solver використовує для гнучких справ
при первинному плануванні, лише експортований і перевикористаний, а не
продубльований). Немає вільного слоту → задача йде в `overflow` з
`reason: "no_slot"` (той самий шлях, яким сам solver обробляє непоміщені справи).

Відповідь (200) — повний `SolverResult`, як `move`/`complete`. Наслідок:
`type` ("fixed"/"flexible") редагована задача **зберігає** — ручна правка часу
не змінює її природу для майбутніх перепланувань.

## 20. Ручний колір задачі, "Перенести → сьогодні", чіпи prefill (Етап 5, раунд 4)

**Колір задачі** — чисто візуальний, НЕ категоризація (LLM як і раніше нічого
не класифікує). За замовчуванням клієнт рахує детермінований хеш
`normalizeTaskKey(title)` → 1 з 6 (`lib/color.ts`: `resolveColorIndex`, чиста
функція, без `lib/db` — цей файл імпортують клієнтські компоненти). Людина може
свідомо закріпити свій колір у `TaskDetailSheet` — тоді override з БД виграє.

`taskColors` (`task_colors`) — точна копія `durationOverrides`: `user_id`,
`task_key`, `color_index` (1-6), `unique(user_id, task_key)`.

**`GET /api/task-color`** → `{ "colors": { "<taskKey>": 3, ... } }` — усі
override користувача одним запитом, підвантажується в `Planner.tsx` при
монтуванні. **`POST /api/task-color`** — `{ "title": "...", "color_index": 1..6 }`,
upsert (inline у роуті — на відміну від `duration-override`, тут лише один
викликач, спільний `lib/*.ts`-хелпер не виправданий).

Обсяг: тільки авторизований перегляд. Публічна `share/[token]` сторінка не має
сесії → бачить лише авто-хеш, override туди не підвантажується.

**`POST /api/plan/move-to-free-slot`** — вхід `{ date, title }` (як `move`).
Знаходить задачу, шукає `findFreeSlot(duration_min, occupied)` серед решти
розкладу дня (той самий виклик, що вже використовує `reschedule` для
конфліктів — не нова логіка). Знайдено → оновлює `start`, повертає повний
`SolverResult` (200). Не знайдено → `409`
`{ "error": "no_free_slot", "message": "..." }`.

Раунд 5: шукає спершу в `schedule` (репозиція — "Перенести → сьогодні" з
`TaskDetailSheet`), якщо там нема — у `overflow` за `normalizeTaskKey`
("Вставити у вільний час" на рядку overflow-задачі, `app/ScheduleView.tsx`).
Знайдено в overflow → `splice` з `overflow`, `push` у `schedule` як
`{ title, start, duration_min, type: "flexible" }` (розміщено автоматично,
не мало явного часу від людини). Той самий контракт відповіді для обох
джерел — один роут, не два майже ідентичних.

**`GET /api/routine`** — форма відповіді змінена: `{ "items": string[] }`
замість `{ "prefill": string }` (Етап 2 — розділ 9). Той самий список
(carry з останнього дня + рутина ≥2 днів, дедуп), але масив, не об'єднаний
рядок — фронтенд рендерить кожен елемент чіпом-чекбоксом
(`app/TaskInputSheet.tsx`), а не підставляє в текст для ручного редагування.

## 21. Додати задачу на сьогодні (Етап 5, раунд 6)

FAB-модалка отримала перемикач "Сьогодні"/"Завтра" (`app/TaskInputSheet.tsx`,
дефолт "Завтра" — вечірній сценарій не зачеплений). "Завтра" — без змін,
`POST /api/plan` (повний перезапис дня). "Сьогодні" — **інкрементне
додавання** в уже існуючий розклад, не перезапис: `POST /api/plan/add-today`,
вхід `{ "text": string, "resolve_conflict"?: boolean }`.

Дата — тільки серверна (`dateStringInTz(timezone, 0)`, як
`api/plan/today`), клієнт її не передає. Дві гілки:
- **Нема рядка `daily_plans` на сьогодні** → `planAndSaveDay` (той самий
  виклик, що звичайний перший план дня) → 200, повний `SolverResult`.
- **Є рядок** → `parseTasks(text, today)` (LLM парсить лише новий текст) →
  `placeNewTasks` (`lib/solver/addTasks.ts`) домержовує в existing
  `SolverResult`, мутуючи на місці. Конфлікт нової фіксованої задачі з
  існуючою і `!resolve_conflict` → `409 { "error": "time_conflict", "conflicts": [{newTitle, withTitle, withStart, withDurationMin}] }`,
  нічого не збережено. `resolve_conflict: true` → існуючі конфліктні
  задачі зсуваються через `findFreeSlot` (розділ 19), нова стає на
  заявлений час. 200 → повний `SolverResult`.

`placeNewTasks` перевикористовує лише вже експортовані примітиви
`lib/solver/solve.ts` (`overlaps`, `findFreeSlot`, `placeFlexible`) —
жодної нової логіки розкладання, тільки оркестрація. Гнучкі нові задачі
без вільного місця → `overflow` (`reason: "no_slot"`), той самий шлях, що
"Вставити у вільний час" (розділ 20). `done`/`moved`-статуси існуючих
задач не чіпаються.
`computePrefill` (`lib/routine.ts`) відповідно повертає `string[]`.

**Не раніше за "зараз"** — обидві гілки передають `minutesInTz(timezone)`
(`lib/timezone.ts`) як нижню межу для гнучких задач: `solve()` і
`placeNewTasks` отримали опційний параметр `minStartMinutes` (за
замовчуванням `0` — вечірнє планування на завтра не зачеплене), який
підіймає нижню межу вікна розміщення гнучкої справи (і зсуву конфліктної
існуючої задачі через `findFreeSlot`) до поточного моменту дня. Без цього
FAB "Сьогодні" міг поставити щойно додану гнучку задачу вранці (07:00),
навіть якщо зараз, наприклад, 11:12 — реальний живий баг, знайдений
користувачем.

## 22. Попап "не вистачило вільного часу" (Етап 5, раунд 7)

Реальний живий баг: коли конфліктну задачу зсуває через `findFreeSlot`
(розділ 19/21) і вільного місця не знаходиться, вона падає в `overflow` з
`reason: "no_slot"` — **мовчки** з погляду людини. Банер підтвердження
("Перенести її на вільний час?") виглядає як обіцянка успіху, хоча solver
чесно міг не знайти куди її поставити.

`placeNewTasks` (`lib/solver/addTasks.ts`) тепер повертає при успіху
`{ ok: true, displaced: string[] }` замість просто `{ ok: true }` —
`displaced` це назви задач, які довелось прибрати зі `schedule` в
`overflow` за час цього виклику. `POST /api/plan/reschedule` рахує той
самий список локально (`displaced: string[]`) і, як і `add-today`, додає
його поруч із рештою `SolverResult` у відповідь: `{ ...SolverResult,
"displaced": string[] }` — не окрема обгортка, щоб фронтенд і далі міг
кастити відповідь як `SolverResult` без змін для решти полів.

`app/Planner.tsx`: новий стан `displacedNotice: string[] | null`.
`onAddToday` перевіряє `data.displaced` після успішного збереження;
`TaskDetailSheet`'s `onSaved` отримав другий опційний аргумент
`displaced?: string[]`. Непорожній список → попап (той самий
`.modal-backdrop`/`.modal-panel`, що інші модалки — не `alert()`, щоб не
випадати з дизайн-мови застосунку): "«X» довелось перенести в «Не влізло
сьогодні» — вільного часу поруч не знайшлося." + кнопка "Зрозуміло".

## 23. Видалення рутинних задач із пропозицій (Етап 5, раунд 8)

Поріг рутини (≥2 з останніх 7 днів, розділ 9) уже сам "забуває" старе —
ковзне вікно. Те, що виглядало як переобсяг пропозицій, насправді був
**carry** (усе незакрите з учорашнього дня, без порогу). Людина хоче
постійно прибирати конкретну назву з пропозицій незалежно від джерела
(carry чи routine).

`ignored_routines` (`lib/db/schema.ts`) — точна копія `task_colors` без
"значення": `user_id`, `task_key`, `unique(user_id, task_key)`. Сама
наявність рядка — прапорець "не пропонувати".

`computeRoutine`/`computePrefill` (`lib/routine.ts`) отримали третій
опційний параметр `ignored: Set<string> = new Set()` (normalizeTaskKey-
ключі) — фільтрує і carry (`uniqueTitles`), і кандидатів рутини до
підрахунку `count`. Не нова логіка рутини, лише фільтр на вже існуючих
шляхах.

**`GET /api/routine`** підвантажує `ignored_routines` користувача одним
запитом поруч із `daily_plans`, будує `Set`, передає в `computePrefill`.

**`POST /api/routine/ignore`** (новий) — вхід `{ "title": string }` →
`normalizeTaskKey` → `insert ... onConflictDoNothing` (ідемпотентно).
Відповідь `{ ok: true }`. Без GET — клієнту потрібен лише вже
відфільтрований `/api/routine`, не сирий ігнор-список.

Фронтенд: кожен пункт дропдауна "Рутинні задачі" (`app/TaskInputSheet.tsx`)
отримав кнопку-кошик поруч із чекбоксом (не всередині `<label>`, щоб клік
не перемикав вибір) → модалка підтвердження (той самий шаблон, що попап
"Не вистачило вільного часу", розділ 22) → підтвердження викликає
`Planner.tsx`'s `onIgnoreRoutineItem`, який зберігає на бекенді й
оптимістично прибирає назву з `routineItems`/`selectedRoutine`.

Ігнор **постійний**, не "на сьогодні" — свідомий компроміс. Впливає лише
на обчислення підказки: ручний ввід тієї самої назви в textarea і далі
успішно планується як завжди.

## 24. "Завтра" — інкрементне домержування замість повного перезапису (Етап 5, раунд 9)

Реальний живий баг: людина вписує задачі по одній протягом вечора — кожен
виклик "Розкласти план" (`POST /api/plan`) робив повний перезапис дня
(`planAndSaveDay`), і попередньо введена задача губилась, якщо не потрапила
в чіпи рутини. `POST /api/plan` тепер має той самий дуал-режим, що вже має
`POST /api/plan/add-today` (розділ 21): перевіряє, чи є рядок `daily_plans`
для цільової дати (`today` з тіла запиту — те саме поле, що й раніше, і далі
приходить від клієнта, не рахується на сервері — це вже була поведінка
роута, не змінено).

- **Нема рядка** → як завжди, `planAndSaveDay(userId, text, today)`, повний
  план з нуля.
- **Є рядок** → `parseTasks(text, today)` (LLM парсить лише новий текст) →
  `placeNewTasks` (`lib/solver/addTasks.ts`, той самий виклик, що
  `add-today`) домержовує в existing, БЕЗ `minStartMinutes` (default `0`) —
  "завтра" не має обмеження "не раніше за зараз", на відміну від "сьогодні".
  Конфлікт і без `resolve_conflict` → `409 { error: "time_conflict",
  conflicts }`. Успіх → `{ ...SolverResult, displaced: string[] }`
  (той самий формат, що `add-today`/`reschedule` — попап "Не вистачило
  вільного часу", розділ 22, працює й тут без жодних змін).

**`placeNewTasks` тепер не дублює вже наявні назви**: на початку збирає
`Set` нормалізованих ключів з `existing.schedule` (без "moved") +
`existing.overflow`, і пропускає (мовчазний no-op, без помилки й без запису
в overflow) будь-якого нового кандидата, чий ключ вже там є. Потрібно саме
для "Завтра" — чіпи рутини лишаються позначеними між відкриттями модалки
(`selectedRoutine` у `Planner.tsx` не скидається після сабміту), тож без
цього фільтра повторний виклик з тими самими чіпами додав би їх ще раз.
Дедлайни не чіпаємо (чіпи рутини їх не включають).

`app/Planner.tsx`'s `onPlan` тепер повертає той самий формат, що
`onAddToday`: `{ok, message?, conflicts?}` замість `Promise<boolean>`.
`app/TaskInputSheet.tsx`: банер конфлікту (раніше лише для вкладки
"Сьогодні") тепер спільний для обох вкладок — той самий локальний стан
`conflict`, кнопка підтвердження викликає `handleAddToday(true)` або
`handlePlan(true)` залежно від активної вкладки.

## 25. Велика літера в назвах задач (Етап 5, раунд 9)

Чисто візуальне: `.cap-first::first-letter { text-transform: uppercase; }`
(`app/globals.css`) — НЕ `text-transform: capitalize` (той міняє регістр
кожного слова, а не лише першої літери речення). Застосовано на назвах у
`app/ScheduleView.tsx`: timeline-картка, рядок overflow, рядок дедлайну.
Збережений текст, `normalizeTaskKey`, пошук і дедуп — усе працює з
оригінальним регістром, як і раніше; змінюється лише те, що бачить око.
Поле "Назва" в `TaskDetailSheet.tsx` (редагування) навмисно без змін —
там людина бачить і редагує сирий текст.

## 26. Резервна модель при перевантаженні LLM (Етап 5, раунд 10)

Реальний живий інцидент: `gemini-3.5-flash-lite` (основна модель,
`lib/parser/parseTasks.ts`) почала стабільно повертати `503 UNAVAILABLE:
"currently experiencing high demand"` — підтверджено напряму (curl/SDK) і
широко відомо на форумі розробників Google AI за той самий період, не
специфічно для цього проєкту/ключа.

`FALLBACK_MODEL = "gemini-3.5-flash"` (те саме покоління, повноцінний
"flash", окрема квота) — `callWithFallback` спершу завжди пробує основну
модель; лише якщо помилка — `ApiError` зі `status` `503` чи `429`
(`isOverloaded`) — одна спроба резервною. Інші збої (мережа, невірний
ключ, 400) fallback не чіпає, одразу `ServiceError`, як і раніше.

**Без стану між викликами** — це свідомий вибір, не спрощення: кожен
виклик `parseTasks` завжди починає з основної моделі. Щойно вона знову
запрацює, наступний-таки запит піде через неї, без потреби "повертати"
щось вручну чи запам'ятовувати, яка модель зараз активна.

Не порушує "без другої моделі для перевірки виводу" (розділ "Архітектурне
правило") — резервна модель виконує ТУ САМУ роботу парсингу, коли основна
недоступна, а не звіряє результат основної.

**Продовження того самого інциденту**: без явного ліміту окремий виклик моделі
міг висіти 40+ секунд перед тим, як зрештою впасти (спостережено напряму) —
цього часу вистачає, щоб з'їсти весь бюджет serverless-функції на Vercel і
отримати сиру помилку платформи замість зрозумілого `ServiceError`. Фікс
фактично зробив це ГІРШЕ: два послідовні виклики (основна + резервна), кожен
потенційно по 40+ секунд.

`CALL_TIMEOUT_MS = 10_000` — кожен виклик `callModel` тепер несе
`config.abortSignal: AbortSignal.timeout(CALL_TIMEOUT_MS)` (стандартний Web
API, підтримується SDK напряму). Таймаут кидає `DOMException` з
`name === "AbortError"` — `isOverloaded` тепер визнає і це (поруч із
503/429) як привід спробувати резервну модель, не лише явні HTTP-коди.
Найгірший випадок — 2 виклики × 10с ≈ 20с, замість потенційно необмеженого
очікування.

`app/api/plan/route.ts` і `app/api/plan/add-today/route.ts` (єдині виклики
`parseTasks`) отримали явний `export const maxDuration = 30` — не покладаємось
на платформний дефолт, якого можна не знати заздалегідь; 30с дає запас понад
розрахований найгірший випадок ~20с.

## 27. Дропдаун дати "Сьогодні/Завтра" (Етап 5, раунд 11)

Після планування "завтра" повернутись до сьогоднішнього перегляду не було
як — лише повне перезавантаження сторінки. Корінь: у `Planner.tsx` не було
ЄДИНОГО джерела істини "який день зараз на екрані", окремого від "що
востаннє відредаговано". `viewingToday` тепер саме це джерело — завжди
актуальне, не побічний ефект.

**`GET /api/plan/tomorrow`** (новий) — точна копія `GET /api/plan/today`
(`getSession` → пояс користувача → `dateStringInTz(timezone, 1)` → SELECT
з `daily_plans`), відповідь `{ result: SolverResult | null, date }`.

**`app/Planner.tsx`**: `loadView(which: "today" | "tomorrow")` — фетчить
відповідний роут, встановлює `viewingToday` і `result`/`planDate`
(обидва `null`, якщо на цю дату ще нема плану — той самий інваріант, що й
раніше: `planDate` ненульовий лише коли є активний `result`). Викликається
з кліку по чіпах "Сьогодні"/"Завтра" в новому дропдауні заголовка (клік по
даті з `CaretDown`, контрольований `dateMenuOpen`, не `<details>` — щоб
закривати програмно після вибору). Монтування (`useEffect`) НЕ викликає
`loadView` — той самий фетч `/api/plan/today` лишився інлайн у ефекті:
`react-hooks/set-state-in-effect` не дозволяє викликати з ефекту функцію,
що виставляє state; `loadView` — лише для дій за кліком. `viewingToday`
дефолт тепер `true` (був `false`) — початкове завантаження завжди
намагається показати сьогодні, старий дефолт створював секундний хибний
порожній напис "на завтра" до відповіді.

**Два супутні баги того самого коріння, знайдені під час аудиту:**

1. `onDurationSaved={onPlan}` (`ScheduleView`, правка тривалості
   overflow-задачі через `DurationEditor`) — мовчки викликала ПОВНИЙ
   перезапис "завтра" з випадковим вмістом `text`/`selectedRoutine` (стан
   FAB-модалки), незалежно від того, який день переглядається. Проп
   прибрано повністю з виклику `<ScheduleView>` — `onDurationSaved ?? (()
   => {})` вже був дефолтом, збереження override і так незалежне (сам
   `POST /api/duration-override` у `DurationEditor`).
2. `TaskDetailSheet`'s "Перенести…" (Сьогодні·вільний час / Завтра) —
   мало сенс лише для задачі з СЬОГОДНІШНЬОГО розкладу; для задачі,
   відкритої під час перегляду "завтра", обидва варіанти безглузді. Новий
   проп `isToday` (=`viewingToday`) гейтить увесь move-menu-блок; "Зберегти"
   лишається завжди доступним (редагування часу/назви/кольору коректне
   для будь-якої дати).

## 28. Явна тривалість у тексті — `duration_min` (Етап 5, раунд 13)

Мама пише "перенести логопеда (30 хв)" — число повністю ігнорувалось.
Причина в двох місцях: (1) схема виводу LLM (розділ 2) не мала поля для
тривалості, `strictObject` відкидав би будь-яке зайве поле; (2)
`durationFor()` у solver читає `title` лише через regex-словник ключових
слів, не намагається витягти число з тексту.

Це не порушення "LLM не оцінює тривалість" (розділ "Архітектурне
правило") — те правило про заборону ВГАДУВАННЯ. Явно написане людиною
число — факт для ВИТЯГУВАННЯ, той самий клас дії, що вже є для
`fixed_time`/`deadline`.

**Схема** (`lib/parser/schema.ts`): нове поле `duration_min: number | null`
у `TaskSchema` (`.int().min(5).max(480).nullable()`) — обовʼязкове, як
`time_hint` (модель має явно повернути `null`, не пропустити поле).

**Промпт** (`lib/parser/prompt.ts`): новий пункт інструкції (дужки
"(30 хв)", "на 30 хвилин", "1 година", "півгодини" → число хвилин, і
приберати з title; нічого в тексті → `null`, НЕ вгадувати). Усі 9
наявних few-shot пар отримали `duration_min: null`; десятий новий
приклад демонструє саме витягування + вирізання з title.

**Пріоритет у solver** (`lib/solver/solve.ts`, `lib/solver/addTasks.ts` —
обидва мають однакову приватну `resolveDuration`, тепер приймає весь
`Task`, не лише `title`):
```
task.duration_min (свіжий текст ЗАРАЗ)
  → overrides.get(normalizeTaskKey(title)) (збережене "Запамʼятати тривалість")
    → durationFor(title) (словник)
      → DEFAULT_DURATION_MIN (30, фінальний фолбек)
```
Свіжий текст перемагає стару збережену пам'ять — свідоме, щойно ухвалене
рішення важливіше за те, що збережено раніше (продуктове рішення,
підтверджено явно, не технічний дефолт).
